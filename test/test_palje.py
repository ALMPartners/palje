...
def test_palje(mssql_config, mssql_credentials, monkeypatch, capsys):
    inputs = {
        ...
        f"User for {mssql_config[0]}.{mssql_config[1]}. If you wish to use Windows Authentication, hit enter: ": mssql_credentials[
            0
        ],
    }
    passwords = {
        f"Atlassian API token for user {AVAILABLE_USERS[0]['user']}: ": AVAILABLE_USERS[
            0
        ]["password"],
