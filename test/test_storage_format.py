...
import palje.confluence.storage_format as storage_format
...
def test_objects_list_should_return_h1_and_p():
    assert storage_format.objects_list() == f"<h1>Objects</h1><p>{CHILDREN_MACRO}</p>"
...
def test_description_header_should_return_h1_and_p():
    ...
    assert (
        storage_format.description_header("test")
        == f"<h1>Description</h1><p>{description}</p>"
    )
...
def test_description_macro_should_match_string():
    description_macro = storage_format.description_macro("test")
    assert ET.tostring(
        description_macro, encoding="unicode"
    ) == DESCRIPTION_MACRO.format("test")
...
def test_column_table_should_return_h1_and_table():
    assert storage_format.column_table(TABLE_INPUT) == f"<h1>Columns</h1>{TABLE_OUTPUT}"


def test_index_table_should_return_h1_and_table():
    assert storage_format.index_table(TABLE_INPUT) == f"<h1>Indexes</h1>{TABLE_OUTPUT}"


def test_parameter_table_should_return_h1_and_table():
    assert (
        storage_format.parameter_table(TABLE_INPUT)
        == f"<h1>Parameters</h1>{TABLE_OUTPUT}"
    )


def test_dependencies_table_should_return_h1_and_table():
    assert (
        storage_format.dependencies_table(TABLE_INPUT)
        == f"<h1>Dependencies</h1>{TABLE_OUTPUT}"
    )
